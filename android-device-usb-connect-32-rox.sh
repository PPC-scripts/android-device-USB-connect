  ...
  # 1- check if a android device seems to be mounted. If so, offer to unmount it and exit OR to access device
	if [ "$(ls -A "$dir")" ]; then
		...
		#--button Unmount && fusermount -u "$dir" && rm -r "$dir" && exit
			foo=$?
			[[ $foo -eq 1 ]] && echo 'user choosed to Access the android device' && rox "$dir" && exit 1
...
mount_display(){
  ...
  jmtpfs "$dir" &&
   ...
   if  [ "$(ls -A "$dir")" ]; then
	rox "$dir" |& tee -a "${output_b}" && echo "device is mounted!"
   else 
